...
import { useTournament, useTournamentDispatch, ACTIONS } from '../context/TournamentContext';
import Modal from '../components/common/Modal';
import { exportAllBracketsToPDF } from '../services/pdfService';
import { parseCategoriesExcel, generateCategoriesTemplate } from '../services/excelService';
import { createKrtData, encodeKrtFile, validateKrtData } from "../services/krtService";
...
export default function TournamentPage() {
  ...
  const getTotalAthletes = () => {
    return tournament.categories.reduce((sum, cat) => sum + (cat.athletes?.length || 0), 0);
  };
  ...
  const handleAddEvent = () => {
    ...
    const event = {
      ...
      ...newEvent,
      weightMin: newEvent.weightMin ? parseFloat(newEvent.weightMin) : undefined,
      weightMax: newEvent.weightMax ? parseFloat(newEvent.weightMax) : undefined
    };
    setKrtFormData(prev => ({ ...prev, events: [...prev.events, event] }));
    setNewEvent({ name: "", gender: "any", type: "kumite", weightMin: "", weightMax: "" });
  };

  const handleRemoveEvent = (eventId) => {
    setKrtFormData(prev => ({ ...prev, events: prev.events.filter(e => e.id !== eventId) }));
  };
  ...
  const handleExportKrt = async () => {
    ...
    const suggestedName = `${tournament.name.replace(/[^a-zA-Z0-9\u00C0-\u1EF9]/g, "_")}.krt`;
    
    try {
      if (window.electronAPI?.saveKrtFile) {
        const result = await window.electronAPI.saveKrtFile(content, suggestedName);
        if (result.success) {
  ...
  const handleExportKmatch = async () => {
    try {
      const kmatchData = createKmatchData(tournament, tournament.categories || [], kmatchSettings);
      ...
      <div className="container">
        ...
        <header className="page-header">
          ...
          <div className="header-actions">
            <button className="btn btn-secondary btn-lg" onClick={handleOpenKrtModal} title="Xuất file .krt cho HLV đăng ký">
            ...
            <button className="btn btn-secondary btn-lg" onClick={handleOpenKmatchModal} title="Xuất file chấm điểm cho Thư ký">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{marginRight: '8px'}}><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg>
              Xuất (.kmatch)
            </button>
            {tournament.categories.filter(c => c.bracket).length > 0 && (
              <button className="btn btn-secondary btn-lg" onClick={handleExportAllPDF}>
                📄 Xuất tất cả PDF
              </button>
            )}
            <button className="btn btn-secondary btn-lg" onClick={handleDownloadTemplate}>
              📥 Tải mẫu Excel
            </button>
              ...
              <div key={category.id} className="category-card card">
                ...
                <div className="category-info">
                  <span className="badge">
                    {category.gender === 'male' ? '👨 Nam' : category.gender === 'female' ? '👩 Nữ' : '👥 Hỗn hợp'}
                  </span>
                  {category.weightClass && <span className="badge">{category.weightClass}</span>}
                  {category.ageGroup && <span className="badge">{category.ageGroup}</span>}
                </div>
                
                <div className="category-stats">
                  <span>{category.athletes?.length || 0} VĐV</span>
                  <span>•</span>
                  <span className={category.bracket ? 'status-done' : 'status-pending'}>
        ...
        <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="Thêm hạng mục mới">
        ...
        {/* Modal xuất KRT */}
        <Modal isOpen={showKrtModal} onClose={() => setShowKrtModal(false)} title="Xuất file .krt cho HLV">
          <div className="krt-form">
            <p className="krt-description">File .krt chứa thông tin giải đấu và thời gian cho phép HLV nhập danh sách VĐV.</p>
            <div className="form-row">
              <div className="input-group">
                <label className="input-label">Thời gian bắt đầu nhập *</label>
                <input type="datetime-local" className="input" value={krtFormData.startTime} onChange={(e) => setKrtFormData(prev => ({ ...prev, startTime: e.target.value }))} />
              </div>
              <div className="input-group">
                <label className="input-label">Thời gian kết thúc nhập *</label>
                <input type="datetime-local" className="input" value={krtFormData.endTime} onChange={(e) => setKrtFormData(prev => ({ ...prev, endTime: e.target.value }))} />
              </div>
            </div>
                ...
                <div className="events-list">
                  {krtFormData.events.map(event => (
                    <div key={event.id} className="event-item">
                      ...
                      <button type="button" className="remove-event-btn" onClick={() => handleRemoveEvent(event.id)}>✕</button>
                    </div>
                  ))}
                </div>
              )}
              <div className="add-event-form">
                <input type="text" className="input" value={newEvent.name} onChange={(e) => setNewEvent(prev => ({ ...prev, name: e.target.value }))} placeholder="Tên nội dung (VD: Kumite Nam -60kg)" />
                <div style={{display: 'flex', gap: '8px', marginTop: '8px'}}>
                  <select className="input" value={newEvent.gender} onChange={(e) => setNewEvent(prev => ({ ...prev, gender: e.target.value }))}>
                    <option value="any">Tất cả</option><option value="male">Nam</option><option value="female">Nữ</option>
                  </select>
                  <select className="input" value={newEvent.type} onChange={(e) => setNewEvent(prev => ({ ...prev, type: e.target.value }))}>
                    <option value="kumite">Kumite</option><option value="kata">Kata</option>
                  </select>
                  <button type="button" className="btn btn-secondary" onClick={handleAddEvent}>+ Thêm</button>
                </div>
              </div>
            </div>
            <div className="modal-actions">
              <button type="button" className="btn btn-secondary" onClick={() => setShowKrtModal(false)}>Hủy</button>
              <button type="button" className="btn btn-primary" onClick={handleExportKrt}>📤 Xuất file .krt</button>
            </div>
          </div>
        </Modal>

        {/* Modal xuất KMatch */}
        <Modal isOpen={showKmatchModal} onClose={() => setShowKmatchModal(false)} title="Xuất file chấm điểm cho Thư ký">
          <div className="krt-form">
            <p className="krt-description">Tạo file .kmatch chứa thông tin trận đấu để Thư ký nhập điểm.</p>
            <div className="input-group">
              <label className="checkbox-label" style={{display: 'flex', gap: '8px', alignItems: 'center'}}>
                <input type="checkbox" checked={kmatchSettings.scoringEnabled} onChange={(e) => setKmatchSettings(prev => ({ ...prev, scoringEnabled: e.target.checked }))} />
            ...
            <div className="form-row">
              <div className="input-group">
                <label className="input-label">Thời gian bắt đầu nhập</label>
                <input type="datetime-local" className="input" value={kmatchSettings.startTime} onChange={(e) => setKmatchSettings(prev => ({ ...prev, startTime: e.target.value }))} />
              </div>
              <div className="input-group">
                <label className="input-label">Thời gian kết thúc nhập</label>
                <input type="datetime-local" className="input" value={kmatchSettings.endTime} onChange={(e) => setKmatchSettings(prev => ({ ...prev, endTime: e.target.value }))} />
              </div>
            </div>
            <div className="modal-actions">
              <button type="button" className="btn btn-secondary" onClick={() => setShowKmatchModal(false)}>Hủy</button>
              <button type="button" className="btn btn-primary" onClick={handleExportKmatch}>🎯 Xuất file .kmatch</button>
            </div>
          </div>
        </Modal>
      </div>
