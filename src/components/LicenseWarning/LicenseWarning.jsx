...
export default function LicenseWarning({ type, onCancel, onSuccess }) {
  ...
  const handleRequestClick = () => {
    ...
    window.open(`mailto:luuquankarate@gmail.com?subject=${subject}&body=${body}`, '_blank');
  };
    ...
    <div className="license-warning-overlay">
      <div className="license-warning-container">
        ...
        <div className="license-warning-header">
          ...
          <button className="license-warning-close" onClick={onCancel}>×</button>
        </div>
        ...
        <div className="license-warning-body">
          ...
          <div className="license-warning-content" style={{ flex: 1 }}>
            <div className="license-warning-message">
              {type === 'expired' ? (
                <p>
                  <strong>License của bạn đã hết hạn.</strong><br/>
                  Vui lòng cài đặt license mới để tiếp tục sử dụng đầy đủ tính năng.<br/>
                </p>
               ) : (
                 <p>
                  Chưa có bản quyền hợp lệ được cài đặt. <br/>
                  Nếu không cài đặt, phần mềm sẽ chạy ở chế độ <strong>DEMO (Dùng thử 3 ngày)</strong>.
                </p>
               )}
            </div>
              <div className="machine-id-box" style={{ marginTop: '1rem', background: '#e2e8f0', padding: '0.75rem', borderRadius: '4px' }}>
              <div style={{ fontSize: '0.8rem', color: '#64748b', marginBottom: '0.25rem' }}>ID Máy tính của bạn:</div>
              ...
              <div style={{ fontSize: '0.75rem', color: '#64748b', marginTop: '0.25rem', fontStyle: 'italic' }}>
                * Cung cấp ID này cho nhà cung cấp để nhận Key kích hoạt riêng cho máy này.
              </div>
            </div>

            {/* Manual Key Input */}
            <div style={{ marginTop: '1rem' }}>
              <div style={{ fontSize: '0.9rem', fontWeight: '500', marginBottom: '0.5rem' }}>Nhập mã License Key:</div>
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <input 
                  type="text" 
                  value={manualKey}
              ...
              <div style={{ fontSize: '0.75rem', color: '#64748b', marginTop: '0.25rem', fontStyle: 'italic' }}>
                * Nếu Key có khóa theo ID máy, ID phải trùng khớp với ID máy bên trên.
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="license-warning-footer">
          ...
          <button className="license-btn btn-primary-install" onClick={handleInstallClick}>
            Mở file License (.lic)
          </button>
