  ...
  const data = {
    ...
    categories: categories.map((cat) => {
      ...
      return {
        ...
        isTeam: Boolean(cat.isTeam || cat.isTeamEvent || cat.bracket?.isTeamBracket),
        teamConfig: cat.teamConfig || null,
  ...
  if (data.categoryMedals && data.categoryMedals.length > 0) {
    ...
    data.categoryMedals.forEach((medal) => {
      medalData.push([
        ...
        medal.bronze2?.name || "",
        medal.bronze2?.club || "",
        medal.gold?.members?.length ? JSON.stringify(medal.gold.members) : "",
        medal.silver?.members?.length ? JSON.stringify(medal.silver.members) : "",
        medal.bronze1?.members?.length ? JSON.stringify(medal.bronze1.members) : "",
        medal.bronze2?.members?.length ? JSON.stringify(medal.bronze2.members) : "",
      ]);
    });
  ...
  if (data.results && data.results.length > 0) {
    const resultsData = [
      [
        ...
        "Nguồn đăng ký VĐV 1",
        "Người đăng ký VĐV 1",
        "Thời gian đăng ký VĐV 1",
        "Bài quyền VĐV 2",
        "Nguồn đăng ký VĐV 2",
        "Người đăng ký VĐV 2",
        "Thời gian đăng ký VĐV 2",
        "Người thắng",
        "CLB thắng",
        "Ghi chú",
      ],
